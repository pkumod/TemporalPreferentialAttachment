...
public class FibonacciHeap {
  ...
  public static void main(String[] args){
	  ...
	  int testsize=59220000;
	  Entry[] entryList=new FibonacciHeap.Entry[testsize];
	  boolean extracted[]=new boolean[testsize];
	 // int samples[] = {11,18,12,14,19,14,12,13,16,13};
	 // int index = 0;
	 // int nums[] = {0, 3, 2};
	 // int indexes[][] = {{}, {7,7,5}, {4,4}}; 
	  int numInsert=0;
	  for(int i=0;i<testsize;i++)
	  {
		  extracted[i]=false;
		  int degree=(int)(Math.random()*10)+10;
		  //int objNumber=(int)(Math.random()*Integer.MAX_VALUE);
		  entryList[i]=new FibonacciHeap.Entry(degree, i);
		  heap.insert(entryList[i]);
		  //int newKey=entryList[i].getKey()-(int)(Math.random()*10);
		  //heap.decreaseKey(entryList[i], newKey);
		  //heap.decreaseKey(entryList[i], newKey);
		  //heap.decreaseKey(entryList[i], newKey);
		  //System.out.println("data:\t" + degree + "\t" + i);
		  numInsert++;
		  if(numInsert%10000000==0)
			  System.out.println("$$$$$$$$$$$$$$$"+numInsert+"$$$$$$$$$$$$$$$");
	  }
	  System.out.println(":-)");
	  Entry current;
	  int numExtracted=0;
	  while((current=heap.extractMin())!=null)
	  {
		  numExtracted++;
		  if(numExtracted%10000000==0)
			  System.out.println("***************"+numExtracted+"******************");
		 // System.out.println(current.getKey()+"	"+current.getObject());
		  extracted[(Integer)current.getObject()]=true;
		  int num=10;
		  //System.out.println("num:\t" + num);
		  for(int x=0;x<num;x++)
		  {
			  int decreaseIt=(int)(Math.random()*testsize);
			  //System.out.println("decreaseIt:\t" + decreaseIt);
			  if(extracted[decreaseIt]==false)
			  {
				  int originalKey=entryList[decreaseIt].getKey();
				  heap.decreaseKey(entryList[decreaseIt], originalKey-1);
				  //entryList[decreaseIt].decreaseKey();
			  }  
		  }
		  //index ++;
	  }
